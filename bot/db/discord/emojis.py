...
from emoji import demojize, distinct_emoji_list
...
class MessageEmojiMapper(Mapper[Message, Sequence[Columns]]):
    ...
    async def map(self, obj: Message) -> List[Columns]:
        regular = self._map_discord(obj, animated=False)
        animated = self._map_discord(obj, animated=True)
        unicode = await self._map_unicode(obj)
        return regular + animated + unicode

    def _map_discord(self, message: Message, animated: bool) -> List[Columns]:
        return [(int(emoji_id), emoji_name, self.to_url(int(emoji_id), animated=animated), animated)
                for (emoji_name, emoji_id) in distinct_emoji_list(message.content)]

    async def _map_unicode(self, message: Message) -> List[Columns]:
        emojis = distinct_emoji_list(message.content)
